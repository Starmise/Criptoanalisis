...
public:
  ...
  */
  std::string 
  decrypt(const std::string& encrypted, const std::string& key, unsigned int seed) {
    ...
    std::vector<size_t> inverseIndex(n);
    for (int i = 0; i < n; ++i) {
      inverseIndex[index[i]] = i;
    }

    std::string unshuffled(n, ' ');
    for (int i = 0; i < n; ++i) {
      unshuffled[i] = encrypted[inverseIndex[i]];
    }

    return XORCipher(unshuffled, key);
  }
